...
int main( int argc, char *argv[] )
{
	...
	if (argc != 2)
    {
		...
		printf("This tool depends on adb to be in your path.\n");
		printf("This tool comes with NO warranties provided.\n\n");
		printf("usage: %s output.img\n", argv[0] );
        exit(1);
    }

	printf("Grabbing started...\n");

	// create and get image
	sysexec("adb shell dd if=/dev/block/platform/omap/omap_hsmmc.0/by-name/radio of=/sdcard/raw.img");
	sysexec("adb pull /sdcard/raw.img .");
    ...
    fprintf(stdout, "Radio grabbed successfully.\n");
	...
	// baseband-name location.
	nameOffset = 0x009fefc0;
	if (fileLen > nameOffset)
	{
		fprintf(stdout, "Detected baseband name: %s\n", (buffer+nameOffset));
	}
	...
	// attempt to clean up
	sysexec("adb shell rm /sdcard/raw.img");
	remove("raw.img");
